...
namespace Game
{
    [AddComponentMenu("Input/On-Screen New Joystick")]
    [InputControlLayout]
    public class InputJoystick : OnScreenControl, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        ...
        [SerializeField]
        private float _multiple = 0.5f;
        
        [SerializeField]
        private float _switchDirection = 0.2f;
        ...
        private void Update()
        {
            ...
            if (Mathf.Abs(direction.x) > _switchDirection && Mathf.Abs(direction.y) > _switchDirection)
            {
                x = _multiple * (direction.x > 0 ? 1f : -1f);
                y = _multiple * (direction.y > 0 ? 1f : -1f);
            }
            else if (Mathf.Abs(direction.x) > Mathf.Abs(direction.y))
            {
                x = 1f * (direction.x > 0 ? 1f : -1f);
                y = 0f;
            }
            else if (Mathf.Abs(direction.x) < Mathf.Abs(direction.y))
            {
                x = 0f;
                y = 1f * (direction.y > 0 ? 1f : -1f);
            }
            
            SentDefaultValueToControl();
            SendValueToControl(new Vector2(x, y));
        }
